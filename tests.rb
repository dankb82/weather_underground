...
class CurrentHurricane
  def initialize(zipcode)
    @hurricane = JSON.parse(File.read("current_hurricane.json"))
  end
end
...
class WeatherUnderground < Minitest::Test
  ...
  def test_current_hurricanes
    ch = CurrentHurricane.new(27616)
    assert_equal ["Invest 90C", "Tropical Cyclone Winston"], ch.current_hurricanes
  end
